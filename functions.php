...
class func{
    public static function checkLoginState($dbh){
        ...
        if(isset($_COOKIE['usuario_id']) && isset($_COOKIE['token'])){
            ...
            if($row['usuario_id']>0){
                if(
                    $row['usuario_id'] == $_COOKIE['usuario_id'] &&
                    $row['token'] == $_COOKIE['token']
                ){
                    if(
                        $row['usuario_id'] == $_SESSION['usuario_id'] &&
    ...
    public static function getEstudiantes($dbh){
        if($_SESSION['usuario_tipo']=='psicologo'){
            ...
            $query1 = 'SELECT * FROM `co-talleristas` WHERE id_psicologo = :usuario_id;';
            ...
            $query2 = 'SELECT * FROM `estudiantes` WHERE `id_co-tallerista`=:usuario_id ;';
            $stmt = $dbh->prepare($query2);
            $stmt ->execute(array(':usuario_id'=>$id));
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($rows as $row){
                $nombre=$row['nombre'];
                $grado=$row['grado'];
